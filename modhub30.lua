...
local GiggleTool = Items:CreateButton({
   Name = "Giggle1!1!1!",
   Callback = function()
...
local GiggleShears = Items:CreateButton({
   Name = "Giggle Shears",
   Callback = function()
loadstring(game:HttpGet("https://raw.githubusercontent.com/Teilsthebfdifan/Guiding-modhub/refs/heads/main/Cheeto%20shears%20fr"))()
Rayfield:Notify({
   Title = "Items",
   Content = "Get any shears , normal or exploits , then use this!!!",
   Duration = 10,
   Image = 73291096617811,
})
   end,
})
local GuidingShears = Items:CreateButton({
   Name = "Guiding Shears",
   Callback = function()
...
local CuriousShears = Items:CreateButton({
   Name = "Curious Shears",
   Callback = function()
local function modifyCuriousShears(tool)
    for _, descendant in ipairs(tool:GetDescendants()) do
        if descendant:IsA("BasePart") then
            if descendant.Name ~= "RushMoving" and descendant.Name ~= "AmbushMoving" and descendant.Name ~= "FigureRig" and descendant.Name ~= "Eyes" then
                descendant.Color = Color3.fromRGB(255, 255, 0)
                descendant.Material = Enum.Material.Neon
            end
        elseif descendant:IsA("ParticleEmitter") and descendant.Name == "DustEmitter" then
            descendant.Texture = "rbxassetid://99203060281323"
...
local function highlightSpecificModels()
    for _, descendant in ipairs(game:GetDescendants()) do
        if descendant:IsA("Model") then
            for _, part in ipairs(descendant:GetDescendants()) do
                if part:IsA("BasePart") then
                    if descendant.Name == "DoorFake" or descendant.Name == "RushMoving" or descendant.Name == "AmbushMoving" or descendant.Name == "FigureRig" or descendant.Name == "Eyes" then
                        createHighlight(part, Color3.fromRGB(255, 0, 0))
                    elseif descendant.Name == "Door" then
...
local function onToolAdded(tool)
    if tool.Name == "Shears" then
        tool.Name = "Curious Shears"
        tool.ToolTip = "Curious Shears"
        if tool:FindFirstChild("Handle") then
            modifyCuriousShears(tool)
        end
        tool.TextureId = "rbxassetid://125185951743980"
    end
end

local function monitorToolAdded()
    game.Players.LocalPlayer.Backpack.DescendantAdded:Connect(function(child)
        if child.Name == "Shears" then
            onToolAdded(child)
        end
    end)
...
local function handleHealthDecrease()
    wait(3600)
    local player = game.Players.LocalPlayer
    if player and player.Character and player.Character:FindFirstChild("Humanoid") then
        ...
        if game.ReplicatedStorage:FindFirstChild("GameStats") then
            if game.ReplicatedStorage.GameStats:FindFirstChild("Player_"..player.Name) then
                game.ReplicatedStorage.GameStats["Player_"..player.Name].Total.DeathCause.Value = "Curious Shears"
            end
        end
    end
end
...
game.DescendantAdded:Connect(function(descendant)
    if descendant:IsA("BasePart") then
        if descendant.Parent and descendant.Parent:IsA("Model") then
            local modelName = descendant.Parent.Name
            if modelName == "DoorFake" or modelName == "RushMoving" or modelName == "AmbushMoving" or modelName == "FigureRig" or modelName == "Eyes" then
                createHighlight(descendant, Color3.fromRGB(255, 0, 0))
            elseif modelName == "Door" then
                createHighlight(descendant, Color3.fromRGB(255, 255, 0))
            end
        end
    end
end)
...
local MischievousShears = Items:CreateButton({
   Name = "Mischievous Shears",
   Callback = function()
...
local GuidingLightTool = Items:CreateButton({
   Name = "Guiding Light Tool (buggy)",
   Callback = function()
local Players = game:GetService("Players")
...
local function createTool()
    local tool = Instance.new("Tool")
    tool.Name = "Guiding Light"
    tool.TextureId = "rbxassetid://76111955799477"
    ...
    local mainParticle = Instance.new("ParticleEmitter")
    mainParticle.Name = "MainParticle"
    mainParticle.Texture = "rbxassetid://76111955799477"
    mainParticle.Size = NumberSequence.new(1)
    mainParticle.Speed = NumberRange.new(0)
    mainParticle.Rate = 15
    mainParticle.Lifetime = NumberRange.new(1)
    mainParticle.Parent = handle

    local secondaryParticle = Instance.new("ParticleEmitter")
    secondaryParticle.Name = "SecondaryParticle"
    secondaryParticle.Texture = "rbxassetid://2581223252"
    secondaryParticle.Size = NumberSequence.new(0.5)
    secondaryParticle.Speed = NumberRange.new(5, 15)
    secondaryParticle.Rate = 0
    secondaryParticle.Lifetime = NumberRange.new(0.5, 1)
    secondaryParticle.Color = ColorSequence.new(Color3.fromRGB(0, 255, 255))
    secondaryParticle.Parent = handle

    local sound = Instance.new("Sound")
    sound.SoundId = "rbxassetid://9125986205"
    sound.PlaybackSpeed = 0.81
    sound.Looped = true
    sound.Volume = 1
    sound.Parent = handle
...
local function highlightDoor()
    for _, descendant in ipairs(game:GetDescendants()) do
        if descendant.Name == "Door" and not descendant:FindFirstChild("Highlight") then
            local highlight = Instance.new("Highlight")
...
local function monitorLights()
    game:GetService("RunService").RenderStepped:Connect(function()
        ...
        if character and character:FindFirstChild("HumanoidRootPart") then
            local tool = character:FindFirstChild("Guiding Light")
            if tool and tool:FindFirstChild("Handle") then
                local handle = tool.Handle
                local light = handle:FindFirstChild("PointLight")
                local foundModels = false
                for _, descendant in ipairs(game:GetDescendants()) do
                    if descendant.Name == "RushMoving" or descendant.Name == "AmbushMoving" or descendant.Name == "FigureRig" or descendant.Name == "Eyes" then
                        foundModels = true
                        break
                    end
                end
                if light then
                    light.Enabled = not foundModels
                end
            end
        end
    end)
...
local function processParticles()
    game:GetService("RunService").RenderStepped:Connect(function()
        local character = LocalPlayer.Character
        if character and character:FindFirstChild("HumanoidRootPart") then
            local tool = character:FindFirstChild("Guiding Light")
            if tool and tool:FindFirstChild("Handle") then
                local handle = tool.Handle
                for _, emitter in pairs(handle:GetChildren()) do
                    if emitter:IsA("ParticleEmitter") and emitter.Name == "MainParticle" then
...
local KeyTracker = Items:CreateButton({
   Name = "Key Tracker",
   Callback = function()
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
...
local NightVision = Items:CreateButton({
   Name = "Night Vision",
   Callback = function()
local Tool = Instance.new("Tool")
...
local MischievousCandle = Items:CreateButton({
   Name = "Mischievous Candle",
   Callback = function()
local function modifyCandle(tool)
    local candlePart = tool:FindFirstChild("Candle")
    local handle = tool:FindFirstChild("Handle")

    if tool:IsA("Tool") and candlePart and candlePart:IsA("BasePart") then
        candlePart.Color = Color3.fromRGB(255, 0, 0)

        local mesh = candlePart:FindFirstChildOfClass("SpecialMesh")
        if mesh then
            mesh.TextureId = ""
        end

        local attachOn = candlePart:FindFirstChild("AttachOn")
        if attachOn then
            for _, descendant in ipairs(attachOn:GetDescendants()) do
                if descendant:IsA("Light") then
                    descendant.Color = Color3.fromRGB(255, 0, 0)
                    descendant.Brightness = 1.75
                    descendant.Range = 20
        ...
        local attachOff = candlePart:FindFirstChild("AttachOff")
        if attachOff then
            for _, descendant in ipairs(attachOff:GetDescendants()) do
        ...
        if handle and handle:IsA("BasePart") then
            handle.Color = Color3.fromRGB(139, 0, 0)
        end

        local dustEmitter = candlePart:FindFirstChild("DustEmitter")
        if not dustEmitter then
            dustEmitter = Instance.new("ParticleEmitter")
            dustEmitter.Name = "DustEmitter"
            dustEmitter.Texture = "rbxassetid://112508503632935"
            dustEmitter.Rate = 10
            dustEmitter.Lifetime = NumberRange.new(1, 2)
            dustEmitter.Speed = NumberRange.new(0.5, 1.5)
            dustEmitter.Size = NumberSequence.new(0.5)
            dustEmitter.Parent = candlePart
        end
    end
end

local function highlightDoorFake(player)
    for _, doorFake in ipairs(game:GetDescendants()) do
        if doorFake.Name == "DoorFake" and doorFake:IsA("Model") then
...
local function trackPlayerTools()
    game.Players.PlayerAdded:Connect(function(player)
        player.Backpack.ChildAdded:Connect(function(tool)
            if tool.Name == "Candle" and tool:IsA("Tool") then
                modifyCandle(tool)
                highlightDoorFake(player)
            end
        end)
    end)
...
game.DescendantAdded:Connect(function(descendant)
    if descendant.Name == "Candle" and descendant:IsA("Tool") then
        modifyCandle(descendant)
    end
end)
...
local UnAbleToJump = PlayerScripts:CreateButton({
   Name = "be unable to jump",
   Callback = function()
game.Players.LocalPlayer.Character:SetAttribute("CanJump",false)
   end,
})
local bom = PlayerScripts:CreateButton({
   Name = "plants revival ideas",
   Callback = function()
local Players = game:GetService("Players")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
local function bom()
    ...
    if ReplicatedStorage:FindFirstChild("GameStats") then
        local playerStats = ReplicatedStorage.GameStats:FindFirstChild("Player_"..Players.LocalPlayer.Name)
        if playerStats and playerStats:FindFirstChild("Total") then
            playerStats.Total.DeathCause.Value = "bom"
        end
    end
end
